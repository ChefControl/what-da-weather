    ...
    async fn request_once(
        ...
        let body = serde_json::json!({
            ...
            "messages": [
                {
                    "role": "system",
                    "content": "You are a concise weather-activity advisor. The user names an activity, guidance on how to decide, the current weather, and condition checks ALREADY COMPUTED by the system. Trust the computed MET / NOT MET results exactly - never recompute or second-guess them. Apply the guidance to those results. Respond with a single JSON object of the form {\"recommended\": true|false, \"reasoning\": \"one or two sentences citing the deciding conditions\"} and nothing else. The reasoning must be consistent with the recommended value."
                },
...
pub fn build_prompt(
    ...
    prompt.push_str(
        "Apply the guidance to the computed checks and decide. \
         ...
         sentences citing the deciding conditions\"}",
    );
    prompt
}
...
mod tests {
    ...
    fn prompt_annotates_computed_checks() {
        ...
        let prompt = build_prompt(
            "Matkot",
            "Recommend only if all MET.",
            &conditions,
            &weather,
        );
        assert!(prompt.contains("Guidance: Recommend only if all MET."));
